...
#cd D:/Python & D: & python New_Castle.py



# Переменные из основной игры
Data = datetime.datetime.now()

Tema = "WHITE"

Начальный_бонус_Замки = True # 300 Монет

Цвет_флага_Л = "G"
Цвет_флага_П = "C"

Игра_против_ИИ = False

сложность = 0


# Функции Const
def ЗВУК_КНОПКИ_BACK():

	def КНОПКА(x):
		try:
			playsound("D:/Profile/BACK.mp3")
		except:
			pass
	th = Thread(target=КНОПКА, args=(1, ))
	th.start()

def В_файле(Путь):
	# Возвращает содержимое файла
	filename = open("D:/Profile/" + str(Путь), "r")
	txt = filename.read()
	filename.close()
	return txt
...
def Музыка_меню():
	...
	Зимняя_музыка = {"FantasyKingdom.mp3", "novigod.mp3"}
	Летняя_музыка = ("menu.mp3", "Menu_1.mp3")
	# Зима
	if Data.strftime("%m") == "11" or Data.strftime("%m") == "12" or Data.strftime("%m") == "01":
		# Др создателя
		if Data.strftime("%d") == "27"  and  Data.strftime("%m") == "11":
			Музыка_Pygame('D:/Profile/dr.mp3', -1)
		# Зимняя музыка
		else:
			Музыка_Pygame("D:/Profile/" + Зимняя_музыка[random.randint(0, len(Зимняя_музыка) - 1)], -1)
	# Остальные времена года
	else:
		# Музыка меню
		Музыка_Pygame("D:/Profile/" + Летняя_музыка[random.randint(0, len(Летняя_музыка) - 1)], -1)
	mixer.music.set_volume(0.6)

def Заменить_значение_защищенного_файла(Путь, Замена):
	os.chmod("D:/Profile/" + str(Путь), S_IWUSR | S_IREAD)
	with open("D:/Profile/" + str(Путь), "w") as f:
		f.write(str(Замена))
	os.chmod("D:/Profile/" + str(Путь), S_IREAD | S_IRGRP | S_IROTH)

def Получение_Ачивки(Код_Ачивки, Номер_Папки, Текст):
	# Старый формат кода
	Prof = open("D:/Profile/"+Номер_Папки,"r")
	AHIV = Prof.read()
	Prof.close()
	if AHIV != Код_Ачивки:
		os.system("cls")
		Prof = open("D:/Profile/" + Номер_Папки,"w")
		Prof = open("D:/Profile/" + Номер_Папки,"a")
		Prof.write(str(Код_Ачивки))
		Prof.close()
		if Tema == "Black":
			print(13*"\n"+"                                                "+Fore.BLACK+Back.WHITE+"Ачивка пройдена!"+Fore.RESET+Back.RESET)
		else:
			print(13*"\n"+"                                                "+Fore.WHITE+Back.BLACK+"Ачивка пройдена!"+Fore.BLACK+Back.WHITE)
		try:
			playsound("D:/Profile/ahiv.mp3")
			time.sleep(1)
		except:
			time.sleep(3)
		os.system("cls")
		if Tema == "Black":
			print(13*"\n"+"                                                "+Fore.BLACK+Back.WHITE+Текст+Fore.RESET+Back.RESET)
		else:
			print(13*"\n"+"                                                "+Fore.WHITE+Back.BLACK+Текст+Fore.BLACK+Back.WHITE)
		time.sleep(5)
		os.system("cls")
	AHIV = None
#
...
# Новые версии Замков изменять отсюда
...
def ЗВУК_BUT_1():
	...
	def КНОПКА(x):
		try:
			playsound("D:/Profile/Castle/But_1.wav")
		except:
			pass
...
def Think():
	...
	if В_файле(32) == "1" and Mus != "Think":
		...
		mixer.music.load("D:/Profile/Castle/Think_" + str(Think_n) + ".mp3")
		mixer.music.play()
def Make():
	global Make_n, Mus

	if В_файле(32) == "1" and Mus != "Make":
		Mus = "Make"
		Make_new = Make_n
		...
		mixer.music.load("D:/Profile/Castle/Make_" + str(Make_n) + ".mp3")
		mixer.music.play()

Режим_разработчика = True
...
Last_input = "" # Переменная для моментального вызова функции print_zamok() отвечает за скорость выбора
...
Win_L = None # True - Если победил левый, False - Если правый, 0 - Выход из игры

One = False # Переменная для обнуления ввода после переключий режимов вопросов
...
if Начальный_бонус_Замки:
	Монеты_Л = 300 # Очки за решенные примеры
	Монеты_П = 300
else:
	Монеты_Л = 0
	Монеты_П = 0
...
Цвета_флаги = ["G", "C", "M", "Y"]
random.shuffle(Цвета_флаги)

if Цвет_флага_Л == "0":
	Цвет_флага_Л = Цвета_флаги[0] if Цвета_флаги[0] != Цвет_флага_П else Цвета_флаги[1]

if Цвет_флага_П == "0":
	Цвет_флага_П = Цвета_флаги[0] if Цвета_флаги[0] != Цвет_флага_Л else Цвета_флаги[1]

if Цвет_флага_Л == Цвет_флага_П:
	Цвет_флага_Л, Цвет_флага_П = Цвета_флаги[0], Цвета_флаги[1]
...
if random.randint(0,1) == 1 or Игра_против_ИИ:
	Ходит_Л = True
else:
	Ходит_Л = False

if Режим_разработчика:
	Ходит_Л = True
	...
	if len(Дефолт) != 0:
		...
		for i in range(От, До, Шаг):
			...
			for j in range(Дефолт[i].V):
				...
				# Ходьба тяжелой пехоты
				if Дефолт[i].Name == "Powerful" and Игровое_поле[Дефолт[i].coord + Дефолт_num] == " ":

					Дефолт[i].coord += Дефолт_num
					#
					Игровое_поле[Temp] = " "
					if Игровое_поле[Дефолт[i].coord + 41] == "M":
						if Дефолт[i].hp < 9:
							Дефолт[i].hp += 1
					...
					#
				# Ходьба магов
				elif Дефолт[i].Name == "Mag" and Игровое_поле[Дефолт[i].coord + Дефолт_num] == " ":

					# Магической атакой атакуем войнов с статусом Norm
					if Поиск_война(Инверсия, Дефолт[i].coord + (Дефолт_num + Дефолт_num), 1) and Поиск_война(Инверсия, Дефолт[i].coord + (Дефолт_num + Дефолт_num), 0).status == "Norm":
						Магическая_Атака(Дефолт[i], Поиск_война(Инверсия, Дефолт[i].coord + 2 * (Дефолт_num), 2), False)
						break
					...
					# Если это союзник (Возможен обгон)
						...
						if Дефолт[i].V != 1:
							...
							# Для магического война отдельное условие
							elif Дефолт[i].Name == "Mag" and Поиск_война(Инверсия, Дефолт[i].coord + 2 * (Дефолт_num), 1):
								Магическая_Атака(Дефолт[i], Поиск_война(Инверсия, Дефолт[i].coord + 2 * (Дефолт_num), 2), False)
								break
...
def Магическая_Атака(Магический_воин, Атакуемый, Ульта):
	global Игровое_поле


	for i in range(Магический_воин.coord, Атакуемый.coord, 1 if Ходит_Л else -1):
		Temp = Игровое_поле[i - 82]
		Игровое_поле[i - 82] = "M"
		print_zamok()
		time.sleep(0.1)
		Игровое_поле[i - 82] = Temp

	Звук_heal()


	if Атакуемый.status == "Norm":

		Атакуемый.hp -= 1

		if random.randint(1, 50) == 50:
			Атакуемый.status = "Freeze"
			Атакуемый.i_status = 3

		if Атакуемый.hp <= 0:
			Игровое_поле[Атакуемый.coord - 41] = " "
			Игровое_поле[Атакуемый.coord] = " "
			Игровое_поле[Атакуемый.coord + 41] = "M"
		...
		if Атакуемый.status == "Freeze":
			Звук_разбил_лед()
		elif Атакуемый.status == "Fire":
			Звук_Fire()
...
def Полет_в_замок(k, skin, Бонус): # cd D:/Python & D: & python New_Castle.py 
...
def print_пример(LIST, LIST_X, X = False):
	...
	if not X and Секунда_Сейчас_ЗАМКИ != Секунда_Сейчас():
	...
	print("\n" + 39 * " " + "TIME " + RED + (str(TIME_Z) if not X else "Время вышло") + RES)
	...
	# Вывод примера после окончания времени
		...
		if LIST_X[Global_Выбор - 1] == X and Global_Выбор != 0:
			...
			if Ходит_Л:
				...
				Заменить_значение_защищенного_файла(25, str(int(В_файле(25)) + 1))
			else:
...
def ИИ():
	...
	Веса_Войны = [[4.72374121], [-20.65290992], [4.72374121]]
	...
	if Баланс < Temp and Мои_Деньги >= 25 and Игровое_поле[477] == " ":
		...
		for i in range(477, 473):
			if coord_противник(i) and Мои_Деньги >= 50 and Перезарядка_пунктов_R[2][1] == 0:
				Номер_война_ии = 2
				...
				for i in range(len(Войны_Л) - 1, 0, -1):
					if Войны_Л[i].coord - 1 == Войны_Л[i - 1].coord and Войны_Л[i - 1].status == "Norm":
						...
						if len(Войны_П) != 0:
							if Войны_П[0].coord - 2 * (Войны_П[0].V) <= Войны_Л[i].coord:
								# Лед
								if Мои_Деньги >= 80 and Перезарядка_пунктов_R[1][1] == 0:
									b = 1
		else:
			if len(Войны_П) >= 2:
				b = 1
	...
	if Перезарядка_пунктов_R[0][3] == 0 and Мои_Деньги >= 40:
		for i in range(477, 474, -1):
			if Игровое_поле[i] == Цвет_флага_П:
				break
			elif Игровое_поле[i] == Цвет_флага_Л and (int(Игровое_поле[i + 41]) <= 3 or coord_противник(477)):
				a = [[2]]
...
#Добавить_война(Ходит_Л, 1)
#Войны_Л.append(Воин(5, Образец_война_легкая_пехота_L[1] + 1, Образец_война_легкая_пехота_L[2], Образец_война_легкая_пехота_L[3], Образец_война_легкая_пехота_L[4], Образец_война_легкая_пехота_L[5], Образец_война_легкая_пехота_L[6], Образец_война_легкая_пехота_L[7], Образец_война_легкая_пехота_L[8]))
#Войны_Л.append(Воин(5, Образец_война_легкая_пехота_L[1] , Образец_война_легкая_пехота_L[2] + 1, Образец_война_легкая_пехота_L[3], Образец_война_легкая_пехота_L[4], Образец_война_легкая_пехота_L[5], Образец_война_легкая_пехота_L[6], Образец_война_легкая_пехота_L[7], Образец_война_легкая_пехота_L[8]))
#Образец_война_легкая_пехота_L = [4,466,"Norm","None",3,2,0,0,"Mag"]
#Войны_П.append(Воин(4, Образец_война_легкая_пехота_L[1] + 2, "Norm", "Freeze", Образец_война_легкая_пехота_L[4], Образец_война_легкая_пехота_L[5], 10, "Freeze", Образец_война_легкая_пехота_L[8]))
#Войны_Л.append(Воин(4, Образец_война_легкая_пехота_L[1] + 1, "Norm", "Freeze", 2, Образец_война_легкая_пехота_L[5], 10, "Freeze", "Powerful"))
#Войны_П.append(Воин(5, Образец_война_легкая_пехота_L[1] + 3, "Norm", "Freeze", Образец_война_легкая_пехота_L[4], Образец_война_легкая_пехота_L[5], 10, "Freeze", "Powerful"))
#Войны_П.append(Воин(4, Образец_война_легкая_пехота_L[1] + 3, "Freeze", "Freeze", Образец_война_легкая_пехота_L[4], Образец_война_легкая_пехота_L[5], 10, "Freeze", "Powerful"))
#Войны_П.append(Воин(4, Образец_война_легкая_пехота_L[1] + 4, "Freeze", "Freeze", Образец_война_легкая_пехота_L[4], Образец_война_легкая_пехота_L[5], 10, "Freeze", "Powerful"))
...
if not Режим_разработчика:
	mixer.music.load("D:/Profile/Castle/Beginning.mp3")
	mixer.music.play()
			
	print_zamok()

	Вывод_информации = ""
	if not Игра_против_ИИ:
		Вывод_информации = "\n\n\n" + 45 * " " + "Ходит команда " + Назавние_левой_команды if Ходит_Л else "\n\n\n" + 45 * " " + "Ходит команда " + Назавние_правой_команды
	...
	while TIME_Z > 0:
		...
		if Секунда_Сейчас_ЗАМКИ != Секунда_Сейчас():
			Секунда_Сейчас_ЗАМКИ = Секунда_Сейчас()
			TIME_Z -= 1
			Чат = "Игра через : " + str(TIME_Z) + '             ' + Back.RED + '"5" - Выход' + Back.WHITE + Вывод_информации + Back.WHITE + Fore.BLACK
			print_zamok()
			Развивание_флагов()
			Чат = ""
	TIME_Z = 6
	del Вывод_информации
...
while Win_L == None:
	...
	if Игра_против_ИИ and not Ходит_Л:
		...
		# Вопрос выбора
			...
			if Секунда_Сейчас_ЗАМКИ != Секунда_Сейчас() and INPUT != "":
				Секунда_Сейчас_ЗАМКИ = Секунда_Сейчас()
				Развивание_флагов()
				print_zamok()
				print_menu()
			...
			if INPUT != Last_input and INPUT != "":
				print_zamok()
				print_menu()

			if One:
				Ввод_через_буфер() # Обнуляем пользовательский ввод
				INPUT = None; Last_input = None
				One = False
	...
	# Моментальный выход с отсылочкой ;)
	if keyboard.is_pressed("Alt") and keyboard.is_pressed("F") and keyboard.is_pressed("4"):
		ЗВУК_КНОПКИ_BACK()
		Win_L = "Exit"
		os.system("cls")
		Очистка_Буфера()
	...
	if Режим_разработчика and INPUT == "5":
		exit()


if Win_L != "Exit":

	def print_zamok(Color):
		os.system("cls")
		print(3 * "\n")
		print(54 * " " + Back.RESET + " " + Color + random.randint(10, 13) * " " + Back.WHITE)
		print(54 * " " + Back.RESET + " " + Color + random.randint(6, 9) * " " + Back.WHITE)
		print(54 * " " + Back.RESET + " " + Back.WHITE)
		print(53 * " " + Back.RESET + "  " + Back.WHITE)
		print(52 * " " + Back.RESET + "    " + Back.WHITE)

		print(50 * " " + Back.RESET + " " + Back.WHITE + "  " + Back.RESET + "  " + Back.WHITE + "  " + Back.RESET + " " + Back.WHITE)
		print(50 * " " + Back.RESET + "  " + Back.WHITE + " " + Back.RESET + "  " + Back.WHITE + " " + Back.RESET + "  " + Back.WHITE)

		print(50 * " " + Back.MAGENTA + "  " + Back.RESET + 4 * " " + Back.MAGENTA + "  " + Back.WHITE)

		print(51 * " " + Back.MAGENTA + " " + Back.RESET + 4 * " " + Back.MAGENTA + " " + Back.WHITE)

		print(52 * " " + Back.MAGENTA + " " + Back.RESET + 2 * " " + Back.MAGENTA + " " + Back.WHITE)
		print(52 * " " + Back.MAGENTA + " " + Back.RESET + 2 * " " + Back.MAGENTA + " " + Back.WHITE)

		print(51 * " " + Back.MAGENTA + " " + Back.RESET + 4 * " " + Back.MAGENTA + " " + Back.WHITE)

		print(" " + Back.RESET + 117 * " " + Back.WHITE + " ")

		print()
		
		print(41 * " " + Color + "░██╗░░░░░░░██╗██╗███╗░░██╗" + Back.WHITE)
		print(41 * " " + Color + "░██║░░██╗░░██║██║████╗░██║" + Back.WHITE)
		print(41 * " " + Color + "░╚██╗████╗██╔╝██║██╔██╗██║" + Back.WHITE)
		print(41 * " " + Color + "░░████╔═████║░██║██║╚████║" + Back.WHITE)
		print(41 * " " + Color + "░░╚██╔╝░╚██╔╝░██║██║░╚███║" + Back.WHITE)
		...
		if Игра_против_ИИ:
			print(2 * "\n" + 44 * " " + "Побежден " + ("сложный" if сложность == 1 else "легкий") + " ии")

	if (Игра_против_ИИ and Win_L) or (not Игра_против_ИИ):
		Мой_цвет = Цвет_флага_Л if Win_L else Цвет_флага_П
		...
		if Мой_цвет == "G":
			...
			Мой_цвет = Back.MAGENTA
		elif Мой_цвет == "Y":
			Мой_цвет = Back.YELLOW


		mixer.music.load("D:/Profile/Castle/Win_" + str(random.randint(1, 2)) + "_Cast.mp3")
		mixer.music.play()

		while True:
			print_zamok(Мой_цвет)
			time.sleep(1)
			Ввод_через_буфер()
			if INPUT == "":
				break
	...
	elif Игра_против_ИИ and not Win_L:
		os.system("cls")
		...
		print(51 * " " + "Ии одержал победу")

		print(2 * "\n" + 48 * " " + Fore.WHITE + Back.RED + '"Enter"' + Back.WHITE + Fore.BLACK + ' - Главное меню', end="")

		playsound("D:/Profile/poragenue.mp3")
	...
	# Ачивки и прочие данные в конце игры
	
	# Ачивка 1000 примеров
	if int(В_файле(25)) >= 1000:
		Получение_Ачивки("ZKYEQZ","28","Решить 1 000 примеров") #28 Ачивка номер 11 • Да ты крутой, сынок! (Решить 1000 примеров)						    ZKYEQZ 
